...
**Everything else avoids the problem instead of working around it.** No menu in this app is drawn by the main renderer: the toolbar's Actions/Remove/Layout/Settings dropdowns and each card's own `⋮`/context menu all open in one compact, sandboxed child window (`playerContextMenu.ts`, items from `toolbarMenu.ts` and `playerMenuModel.ts`), which the OS composites above the guest views with no stacking conflict, so nothing has to be hidden for them. Where a menu item can't do the job — a free-text Columns/Per-page value — the menu hands off to an inline field in the toolbar, which sits above `grid-mount` where no player is ever bounded. Per-card tooltips are native `title` attributes for the same reason. The rule this leaves for future work: renderer-drawn UI either lives outside every player's bounds, or it is not renderer-drawn.

**Header height is an input to the bounds loop, not a CSS detail.** `PlayerCardComponent` picks one of four density tiers (`xs`/`sm`/`md`/`lg`) from its own `ResizeObserver`, and each tier renders a *different* toolbar at a different fixed height — so a card can change how tall its header is without the grid around it changing at all. `GridComponent` therefore observes each card's `.player-card__body` directly, not just the grid host: whatever moves or resizes that element — a density change, a window resize, a page flip, a card being removed — re-reports bounds. That is also why the density rules live entirely in the card component and nothing in `GridComponent` or `PlayerManager` knows they exist; the bounds engine reacts to the measured result rather than being told about the CSS.
...
flowchart TD
    ...
    AC -- "players, columnOverride, pageSize, currentPage,<br/>focusedPlayerId, selectedIds, runtimeStates, metrics,<br/>renameTargetId (inputs)" --> GC
    GC -- "focusChange, toggleSelect, remove, navigate,<br/>goBack, goForward, reload, stop, toggleMute,<br/>openDevTools, rename, renameClosed, openMenu (outputs)" --> AC
    GC --> PC1
    GC --> PC2
    GC --> PCn
```

**`AppComponent`** is the state root. It holds every cross-cutting signal — `players`, `focusedPlayerId`, `selectedIds`, `columnOverride`, `pageSize`/`currentPage`, `runtimeStates`, `metrics`, `gpuDisabled`, the whole Add Player modal's state (`modalOpen`, `mode`, build progress) — and is the only component that talks to `window.playerSwarm` directly. Its constructor wires up the two push subscriptions (`onPlayersChanged`, `onPlayerStateChanged`) and the metrics poll, and one `effect()` ties `modalOpen()` to `setOverlayVisible()`. It also owns every menu request: `showMenu()` posts the toolbar's Actions/Remove/Layout/Settings menus, `openPlayerMenu()` posts a card's own menu on behalf of whichever card asked, and `onToolbarMenuAction()` routes each clicked item back to the same method the equivalent inline button would have called — the menu replaces the button, never the behaviour. The compact fleet row in the toolbar (`statusSummary`, `resourceSummary`, `fleetTooltip`) is derived here from the same `playerStatusOf()` the cards use, so the fleet count and the per-card pips can't disagree.

**`GridComponent`** is purely presentational plus the bounds-reporting engine (§6c). It derives `pagedPlayers` (slices `players()` by `pageSize`/`currentPage`), `dims` (column/row count — either `columnOverride`, or `computeGridDimensions()` choosing whichever split puts the resulting cells closest to 16:9 in the space actually measured), `focusedPlayer`/`otherPlayers` (splits the list when something is maximized), and renders one of three template branches: the empty state, the plain paginated grid, or the focus layout (one large `player-focus-main` + a scrollable `player-focus-strip` of thumbnails). It never calls `window.playerSwarm` itself for player actions — every user action is re-emitted upward as an `output()` for `AppComponent` to actually dispatch, keeping IPC calls in one place.

**`PlayerCardComponent`** is the per-player chrome, and it renders a different amount of it depending on how big the card actually is. A `ResizeObserver` on its own host feeds a `density` signal — `lg` (≥560px wide, or expanded, which is always `lg`), `md` (≥360), `sm` (≥210), `xs` below that, with height caps so a wide-but-short card can't spend a third of itself on chrome. Each tier renders a genuinely different toolbar rather than the same controls at a smaller size: `lg` has back/forward, reload/stop, the URL bar, mute, DevTools, expand and remove; `md` drops back/forward and DevTools; `sm` drops the URL bar and mute, keeping selection, status, identity, reload, expand and remove; `xs` shows only a checkbox, the status pip, a short `#id` handle, reload and expand. Whatever a tier drops is reachable from that card's native overflow menu (`⋮`, and right-click anywhere on the header), so nothing becomes unavailable — only less prominent. Also here: inline rename (double-click the label, or "Rename…" in the card menu via `renameTargetId`; `Enter`/blur commits, `Escape` cancels), the status pip (`playerStatusOf()` → running/loading/recovering/error/stopped), an indeterminate load bar drawn inside the header, the error card (shown when `runtimeState().error` is set), and the footer's env/license/instance/metrics readout at `md` and `lg`. Its `.player-card__body` div (tagged `[data-player-id]`) is the empty placeholder the corresponding `WebContentsView` is positioned over — the component renders no actual player content itself.

## 6. Main-process subsystems

- **`PlayerManager`** — the single owner of every guest view's lifecycle. `create()`/`createMany()` (150ms-staggered) build a `Player` record and call `attachView()`, which constructs the `WebContentsView`, sets it hidden, adds it to the shell window, loads the URL, and wires `attachWatchdog()`. The watchdog tracks loading/nav-availability state (via `did-start-loading`/`did-stop-loading`/`webContents.navigationHistory`), persists the live URL on every navigation, and on `render-process-gone` calls `setPlayerError()` + `scheduleAutoRelaunch()` — a capped exponential backoff (1s → 2s → 4s → 8s → 16s, capped at `MAX_AUTO_RELAUNCH_ATTEMPTS = 5` attempts, after which only a manual Reload can recover it). While a retry is pending the runtime state carries `recovering: true`, which is what lets a card distinguish "crashed, retrying" from "crashed, out of retries" instead of showing one undifferentiated error. `getMetrics()` cross-references `app.getAppMetrics()` (system-wide, keyed by OS pid) against each view's `webContents.getOSProcessId()` to build a per-player CPU/memory snapshot. `applyBounds()` is the landing point for every `reportGridBounds` IPC message.
