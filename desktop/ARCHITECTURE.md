...
**Everything else avoids the problem instead of working around it.** No menu in this app is renderer-drawn: the toolbar's Actions/Remove/Layout/Settings dropdowns and each card's own overflow/context menu are all real Electron `Menu.popup()` calls (`toolbarMenu.ts`), composited by the OS above the guest views with no stacking conflict, so nothing has to be hidden for them. Where a native menu item can't do the job — a free-text Columns/Per-page value — the menu hands off to an inline field in the toolbar, which sits above `grid-mount` where no player is ever bounded. Per-card tooltips are native `title` attributes for the same reason. The rule this leaves for future work: renderer-drawn UI either lives outside every player's bounds, or it is not renderer-drawn.
